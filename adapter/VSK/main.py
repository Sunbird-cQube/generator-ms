...
class CollectData:
    def __init__(self):
        ...
        if self.env == 'azure':
            ...
            try:
                self.minio_client = Minio(endpoint=self.minio_endpoint + ':' + self.minio_port,
                                          access_key=self.minio_username, secret_key=self.minio_password,
                                          secure=False)  # set this to True if your Minio instance is secured with SSL/TLS
                self.minio_object_list = self.minio_client.list_objects(self.minio_bucket, prefix=self.input_folder,
                                                                        recursive=True)
            except Exception:
            ...
            try:
                self.file_contents = self.oracle_storage_client.get_object(self.namespace, self.oracle_bucket,
                                                                           self.input_folder)
    ...
    def column_rename(self, data):
        df = pd.read_csv(data,on_bad_lines='skip',lineterminator='\n')
    ...
    def upload_file(self, csv_data, output_file):
        ...
        if self.env == 'azure':  ## uploading file to Azure blob container
            ...
            self.minio_client.put_object(bucket_name=self.minio_bucket,
                                         object_name=self.output_folder + '/' + output_file, data=csv_buffer,
                                         length=len(csv_bytes), content_type='application/csv')
            print(f"Message : File {output_file} uploaded successfully to the folder {self.output_folder}.")
        elif self.env == 'oracle':
            self.oracle_storage_client.put_object(self.namespace, self.oracle_bucket,
                                                  self.output_folder + '/' + output_file, csv_buffer)
            print(f"Message : File {output_file} uploaded successfully to the folder {self.output_folder}.")
        else:
            print(f'Message : Storage type {self.env} is not valid')
